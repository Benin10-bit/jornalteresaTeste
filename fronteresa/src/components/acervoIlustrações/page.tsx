...
function GalleryItem({
    ...
    image: (typeof ilustracoesExemplo)[0];
    onClick: (image: (typeof ilustracoesExemplo)[0]) => void;
}) {
...
export default function AcervoIlustracoes() {
    ...
    const [selectedImage, setSelectedImage] = useState<(typeof ilustracoesExemplo)[0] | null>(null);

    return (
        <div className="flex flex-col min-h-screen" style={{ backgroundColor: "var(--background)" }}>
            <Header />

            <main className="grow px-4 md:px-8 lg:px-12 pb-12" style={{ paddingTop: "clamp(7rem, 10vw, 7rem)" }}>
                ...
                <div className="max-w-7xl mx-auto columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-4 md:gap-6 space-y-4 md:space-y-6">
                    {ilustracoes.map(image => (
                        <div key={image.id} className="break-inside-avoid mb-4 md:mb-6">
                            <GalleryItem image={image} onClick={img => setSelectedImage(img)} />
                        </div>
                    ))}
                </div>
